...
for index in indexs:
    ## When selecting six features, we can get six features by deleting two features
    s = np.linspace(0,7,8)
    sub_index = np.delete(s,[index[0],index[1]])
    ...
    for emb_dim1 in emb_dim[0]: ## could be emoji, glove-twitter or others
        for emb_dim2 in emb_dim[1]: ## could be glove-twiiter or others
            ...
            if os.path.exists('baseline/result/LR_multi6_{}_max.csv'.format(str(item))):
                if newTry:
                    with open('baseline/result/LR_multi6_{}_findC.csv'.format(str(item)), 'w') as f:
                        writer = csv.writer(f)
                        writer.writerow(["feature","C","microF1"])
                    pass
                    checkPoint = False
                else:
                    with open('baseline/result/LR_multi6_{}_max.csv'.format(str(item)), newline='') as csvfile:
                ...
                with open('baseline/result/LR_multi6_{}_max.csv'.format(str(item)), 'w') as f:
                    writer = csv.writer(f)
                    writer.writerow(["feature","C","microF1"])
            ...
            for j in range(-4, 3):
                ...
                with open('baseline/result/LR_multi6_{}_max.csv'.format(str(item)), 'a') as f:
                    writer = csv.writer(f)
                    for row in results:
                        writer.writerow(row)
                results = []
            ...
            if not checkPoint:
                ...
                with open('baseline/result/LR_multi6_{}_findC.csv'.format(str(item)), 'a') as f:
                    writer = csv.writer(f)
