...
for index in indexs_for_three:
    ...
    if index[-1]<7:
        sub_s = s[index[-1]+1:]
        for si in sub_s:
            index_for_four = [int(index[0]), int(index[1]), int(index[2]), int(si)]
            index_for_four.sort()
            indexs_for_four.append(index_for_four)
        pass
...
for index in indexs_for_four:
    ...
    for emb_dim1 in emb_dim[0]: ## could be emoji, glove-twitter or others
        for emb_dim2 in emb_dim[1]: ## could be glove-twiiter or others
            ...
            if os.path.exists('baseline/result/LR_multi4_{}_max.csv'.format(str(item))):
                if newTry:
                    with open('baseline/result/LR_multi4_{}_findC.csv'.format(str(item)), 'w') as f:
                        writer = csv.writer(f)
                        writer.writerow(["feature","C","microF1"])
                    pass
                    checkPoint = False
                else:
                    with open('baseline/result/LR_multi4_{}_max.csv'.format(str(item)), newline='') as csvfile:
                ...
                with open('baseline/result/LR_multi4_{}_max.csv'.format(str(item)), 'w') as f:
                    writer = csv.writer(f)
                    writer.writerow(["feature","C","microF1"])
            ...
            for j in range(-5, 3):
                ...
                with open('baseline/result/LR_multi4_{}_max.csv'.format(str(item)), 'a') as f:
                    writer = csv.writer(f)
                    for row in results:
                        writer.writerow(row)
                results = []
            ...
            if not checkPoint:
                ...
                with open('baseline/result/LR_multi4_{}_findC.csv'.format(str(item)), 'a') as f:
                    writer = csv.writer(f)
