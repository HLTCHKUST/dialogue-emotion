...
'''
txt_file = "lr_multi2-merged.txt" if constant.include_test else "lr_multi2.txt"

# feature_list = [ "emo2vec", "bert", "elmo", "glove-w2v", "deepmoji", "glove-common", "glove-twitter", "emoji",]
feature_list = [ "deepmoji-avg", "elmo"]
...
for index in indexs:
    ...
    for emb_dim1 in emb_dim[0]: ## could be emoji, glove-twitter or others
        for emb_dim2 in emb_dim[1]: ## could be glove-twiiter or others
            ...
            if os.path.exists('baseline/result/LR_multi2_{}_max.csv'.format(str(item))):
                if newTry:
                    with open('baseline/result/LR_multi2_{}_findC.csv'.format(str(item)), 'w') as f:
                        writer = csv.writer(f)
                        writer.writerow(["feature","C","microF1"])
                    pass
                    checkPoint = False
                else:
                    with open('baseline/result/LR_multi2_{}_max.csv'.format(str(item)), newline='') as csvfile:
                ...
                with open('baseline/result/LR_multi2_{}_max.csv'.format(str(item)), 'w') as f:
                    writer = csv.writer(f)
                    writer.writerow(["feature","C","microF1"])
            ...
            for j in range(0,1):
                ...
                for i in range(constant.num_split):
                    ...
                    txtfile.write("###### EXPERIMENT %d when C equals to %f ######\n" % ((i+1),c))
                    txtfile.write("(EXPERIMENT %d) Create the model\n" % (i+1))
                    txtfile.close()
                    ...
                    txtfile = open(txt_file,'a')
                    txtfile.write("(EXPERIMENT %d) microF1 score %f\n" % ((i+1), microF1))
                    txtfile.write("\n--------------------\n")
                    txtfile.close()
                ...
                with open('baseline/result/LR_multi2_{}_max.csv'.format(str(item)), 'a') as f:
                    writer = csv.writer(f)
                    for row in results:
                        writer.writerow(row)
                results = []
            ...
            if not checkPoint:
                ...
                with open('baseline/result/LR_multi2_{}_findC.csv'.format(str(item)), 'a') as f:
                    writer = csv.writer(f)
